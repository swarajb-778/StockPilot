...
const CardPurchaseSummary = () => {
  ...
  const aggregatedData = useMemo(() => {
    ...
    if (timeframe === "weekly") {
      const weeklyMap = new Map<string, { purchases: number; changePercentage: number; count: number; date: string }>();
      ...
      sortedData.forEach(item => {
        ...
        if (weeklyMap.has(weekKey)) {
          ...
          existing.purchases += item.totalPurchased || 0;
    ...
    if (timeframe === "monthly") {
      const monthlyMap = new Map<string, { purchases: number; changePercentage: number; count: number; date: string }>();
  ...
  const totalPurchases = aggregatedData.reduce((acc, curr) => acc + curr.purchases, 0);
            ...
            <div ref={chartRef}>
              <ResponsiveContainer width="100%" height={200} className="px-2">
                ...
                >
                  ...
                  />
                  <Tooltip
                    formatter={(value: number) => [`$${value.toLocaleString("en")}`, "Purchases"]}
